...
void oncelikBul(node *front){
		int enkucuk;
		// En öncelikli 24 elamaný seçme
for(int i =0; i<24 ;i++)
{
	iter=front;
	enkucuk=25;
	// En erken iniþ talep edilen saati bulma
	while(iter != NULL)
	{
	...
	iter=front;
	// Ayný saate iniþ isteyenlerden en öncelikli olnaný bulma
	while(iter != NULL)
	{	
	...
	iter=front;
	// iniþ izni alamayan uçaklarý bir sonraki saate erteleme
	while(iter != NULL)
	{
	...
	// En öncelikli elemaný dosyaya yazdýrma
	iter=front;
	int enk=25;
	while(iter!= NULL){
